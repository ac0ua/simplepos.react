...
function syncProductCategories($conn, $productId, $categories)
{
    try {
        ...
        error_log('syncProductCategories error: ' . $e->getMessage());
    }
}
...
try {
    ...
    $categories = $data['categories'] ?? null; // New multi-category support
    ...
    // Sync categories (use categories array if provided, otherwise use single category)
    syncProductCategories($conn, $productId, $categories ?? $category);
