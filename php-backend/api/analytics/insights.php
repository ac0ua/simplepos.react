...
try {
    ...
    // Sales by category - join with products table to get category
    $stmt = $conn->prepare("
    ...
    // Get products with stock levels (using 'stock' column, not 'stock_quantity')
    $stmt = $conn->prepare("
    ...
    // Low stock items (threshold of 10)
    $lowStockItems = array_filter($inventoryLevels, function($item) {
        return (int)$item['stock_quantity'] <= 10 && (int)$item['stock_quantity'] > 0;
    });
